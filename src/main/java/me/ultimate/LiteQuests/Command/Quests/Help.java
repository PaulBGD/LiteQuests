...
public class Help implements BaseCommand {
   ...
   public void perform(final Player p, final String allArgs, final String[] args) {
      if (args.length == 1) {
         ...
         for (final Map.Entry<String, BaseCommand> entry : MainCommand.commandClasses.entrySet()) {
            final BaseCommand cmd = entry.getValue();
            if (!cmd.isAlias())
         ...
         if (cont) {
            Send.sendMessage(p, "&8 -=-=-=-=-=-=- &0[ &aLiteQuests Admin Help &0] &8-=-=-=-=-=-=-", false);
            Send.sendMessage(p, "&a" + cmdClass.getUsage() + " &8--> &a" + cmdClass.getHelpMessage(), false);
         } else {
